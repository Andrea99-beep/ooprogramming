...
public class DataEntry_v2_Online_Forward extends JFrame implements ActionListener {
	...
	public void load() {
		try {
			...
			while (rs.next()) {
				lm.add(new Mortadella(java.util.UUID.fromString(rs.getString("id")), rs.getDouble("lenght"),
	...
	public void showItem() {
		try {
			tfLenght.setText(Double.toString(lm.get(selectedItem).getLenght()));
			tfDiameter.setText(Double.toString(lm.get(selectedItem).getDiameter()));
			tfWeight.setText(Double.toString(lm.get(selectedItem).getWeigth()));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnInsert) {
			String[] v = JOptionPane.showInputDialog(this, "Insert Mortadella (lenght;diameter;weight;quality)")
					.split(";");
			Mortadella m = new Mortadella(java.util.UUID.randomUUID(), Double.parseDouble(v[0]),
					Double.parseDouble(v[1]), Double.parseDouble(v[2]), v[3]);

			try {
				String query = String.format(
						"INSERT INTO mortadella (id, lenght, diameter, weight, quality) VALUES ('%s', %f, %f, %f, '%s')",
						m.getId().toString(), m.getLenght(), m.getDiameter(), m.getWeigth(), m.getQuality());
		...
		if (e.getSource() == btnRemove) {
			try {
				String query = String.format("DELETE FROM mortadella WHERE id='%s'", lm.get(selectedItem).getId());
				db.executeUpdate(query);
		...
		if (e.getSource() == tfLenght) {
			String query = String.format("UPDATE mortadella SET lenght=%f WHERE id='%s'",
					Double.parseDouble(tfLenght.getText()), lm.get(selectedItem).getId());
			try {
				db.executeUpdate(query);
		...
		if (e.getSource() == cbQuality) {
			String query = String.format("UPDATE mortadella SET quality='%s' WHERE id='%s'",
					cbQuality.getSelectedItem(), lm.get(selectedItem).getId());
			try {
				db.executeUpdate(query);
