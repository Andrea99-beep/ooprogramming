...
/**
 * This is a fairly simple subclass of jawa.awt.JPanel that can display a
 * part of the Mandelbrot set. The interval bounds and the depth limit can
 * be set at run time to select which part you want to be shown.
 */
public class MandelbrotPanel extends JPanel {
	private static final long serialVersionUID = 1L;

	/**
     * Number of non-black colours in the colour table (the last
     * entry is always black, and is not included in this number).
     */
    ...
    /**
     * Initialize a new MandelbrotPanel:
     * Create and fill an IndexColorModel with the colour table and add
     * a Mouse(Motion)Listener to the JPanel. The initial interval is
     * [-2, 0.5] x [-1.25, 1.25].
	...
	this.re_min = re_min; this.re_max = re_max;
	this.im_min = im_min; this.im_max = im_max;
    ...
    /**
     * Perform the Mandelbrot iteration for a single complex coordinate
     * until the iteration terminates or the given depth limit is reached.
    ...
    /**
     * Redraw this component. Recalculates the BufferedImage if needed
     * (the BufferedImage is cached for efficiency reasons) and draws
     * it into the component. This implementation uses a simple 8-bit
     * IndexColorModel and a PackedRaster (the type of raster really
     * doesn't matter here).
     */
    @Override
    ...
    /**
     * MouseListener and MouseMotionListener to allow the user to draw a
     * zoom rectangle with the mouse inside the image (which will then be
     * magnified to show only the selected region).
     */
